...
| **目录容灾回退**             | 目录获取采用三级回退链：官方 `models.dev`（10 秒超时）→ 镜像（`opencodego.modelsDevMirrorUrl`，默认 `https://modelsdev-mirror.onesoft.top/catalog.json`，30 秒超时，请求头携带 `platform: opencode-go-copilot` 及可选 `x-mirror-token`）→ 硬编码兜底模型列表。镜像/兜底命中时按 1 分钟间隔持续重试官方源，官方恢复后自动切回                                                                                                                                                                                                                                                                                                                                                                                                   |
...
> 兜底快照：`src/hardcodedModelList.ts` 内置 2026-08-04 的 opencode-go（24 个）与 opencode 免费（22 个）模型 ID/名称列表，仅作官方目录与镜像均不可达时的最后防线，元数据走覆盖表与保守默认值。
...
├── hardcodedModelList.ts                 # 硬编码兜底模型列表（官方目录与镜像均不可达时的最后防线）
...
| `hardcodedModelList.ts`               | ~90  | 硬编码兜底模型列表：opencode-go（24 个）与 opencode 免费（22 个）模型的 ID/名称快照（2026-08-04），官方目录与镜像均不可达时构建最小目录，元数据走覆盖表与保守默认值                             |
...
从 `https://models.dev/catalog.json` 下载完整模型目录并构建内存索引（完整 ID → 条目 + 短 ID → 条目 + provider → 条目）。内部 `fetchCatalog()` 采用三级回退链：官方源（10 秒超时）→ 镜像（`opencodego.modelsDevMirrorUrl`，30 秒超时，携带 `platform: opencode-go-copilot` 与可选 `x-mirror-token` 请求头）→ 硬编码兜底模型列表（`buildHardcodedCatalog()`，仅含 id/name，provider `api` 留空走内置回退 URL）。1 分钟缓存 TTL（短 TTL 兼作 VS Code 启动时多个并发 `activate()` 调用的去重窗口，同时保证每次激活/刷新均重新拉取目录）。镜像/兜底命中时 `lastLoadFailed=true`，按 1 分钟间隔持续重试官方源，官方恢复后自动切回；兜底命中且内存已有旧目录数据时保留旧数据（比硬编码列表更新），仅更新重试时机。失败时静默保留旧缓存，首次无缓存时初始化为空 Map。
