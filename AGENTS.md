...
| **目录容灾回退**             | 目录获取采用回退链：官方 `models.dev`（10 秒超时）→ 镜像（`opencodego.modelsDevMirrorUrl`，默认 `https://modelsdev-mirror.onesoft.top/catalog.json`，30 秒超时，请求头携带 `platform: opencode-go-copilot` 及可选 `x-mirror-token`）。镜像命中时按 1 分钟间隔持续重试官方源，官方恢复后自动切回                                                                                                                                                                                                                                                                                                                                                                                                   |
...
从 `https://models.dev/catalog.json` 下载完整模型目录并构建内存索引（完整 ID → 条目 + 短 ID → 条目 + provider → 条目）。内部 `fetchCatalog()` 采用回退链：官方源（10 秒超时）→ 镜像（`opencodego.modelsDevMirrorUrl`，30 秒超时，携带 `platform: opencode-go-copilot` 与可选 `x-mirror-token` 请求头）。1 分钟缓存 TTL（短 TTL 兼作 VS Code 启动时多个并发 `activate()` 调用的去重窗口，同时保证每次激活/刷新均重新拉取目录）。失败时静默保留旧缓存。首次无缓存时初始化为空 Map。`lastLoadFailed` 标志追踪上次加载是否失败：失败后至少等待 1 分钟再重试，避免空 Map 长期阻塞后续重试。
